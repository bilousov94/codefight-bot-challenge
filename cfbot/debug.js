/**
 * Created by Valentyn on 5/9/2017.
 */
...
function taskMaker(source, challengeId) {
    var string = '';
    var regeEx = new RegExp('//DB ' + challengeId + '//');
    var substr = '';
    for(var i = 0; i < source.length; i++){


        if (regeEx.test(source[i])){

            substr = source[i];
            string = substr.replace('//DB ' + challengeId + '//', '');
        } else if(source[i].match(/\/\/DB/)) {
